"""Persistent status for single-process desktop reparse jobs."""
...
def queue(notice_id):
    ...
    with get_db() as c:
        ...
        task_id = c.execute("INSERT INTO reparse_tasks(notice_id,status,created_at) VALUES(?,'queued',?)", (notice_id,now_iso())).lastrowid
        return task_id, True
