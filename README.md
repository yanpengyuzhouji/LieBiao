...
后端启动后会通过 GitHub Releases 公开接口检查一次新版本，运行期间每小时由独立线程静默检查；检查失败不提示且不占用采集线程。同一新版本只提示一次，点击右上角通知按钮可打开下载页。可通过 `LIEBIAO_UPDATE_ENABLED=false` 关闭，或用 `LIEBIAO_UPDATE_URL` 指向其他 HTTPS JSON 更新清单。发布新版本时需在 GitHub 创建对应 Release（例如 `v1.2.0`）并上传安装包。
