...
const viewMeta = {
  ...
  notices: ['公告库', '汇总三个平台的最新招标信息，快速确认与跟进。'],
...
function renderNotices() {
  ...
  return `${renderStats()}${renderDataStrip()}<div class="toolbar-card">
    ...
    <div class="filter-row">
      ...
      <label class="select-wrap platform"><select id="platform-filter"><option value="all">全部平台</option><option value="csg" ${appState.platform === 'csg' ? 'selected' : ''}>南方电网</option><option value="ecp" ${appState.platform === 'ecp' ? 'selected' : ''}>ECP2.0</option><option value="sgcc" ${appState.platform === 'sgcc' ? 'selected' : ''}>国网交易专区</option></select></label>
...
function renderOverview() {
  const platformDefinitions = [{ code: 'csg', name: '南方电网', tone: 'green' }, { code: 'ecp', name: 'ECP2.0', tone: '' }, { code: 'sgcc', name: '国网交易专区', tone: 'orange' }];
  ...
  const platformRows = platformCounts.map(platform => `<div class="health-row"><span class="platform-logo ${platform.tone}">${platform.code === 'csg' ? '南' : platform.code === 'ecp' ? 'E' : '国'}</span><span class="health-name">${platform.name}</span><span class="health-bar"><span style="width:${Math.round(platform.count / maxCount * 100)}%"></span></span><strong>${platform.count} 条</strong></div>`).join('');
...
function renderLivePlatforms() {
  const logo = code => code === 'csg' ? '南' : code === 'ecp' ? 'E' : '国';
  return `<section class="panel-card"><div class="panel-head"><div><h3>平台与账号 <span style="color:#9da7b7;font-family:'DM Mono';font-size:10px">${appState.sites.length}</span></h3><p>当前三个平台均按公开采集模式运行，不需要账号。</p></div></div><div class="panel-body"><table class="platform-table"><thead><tr><th>平台</th><th>采集模式</th><th>连通状态</th><th>最近检查（北京时间）</th><th>操作</th></tr></thead><tbody>${appState.sites.map(site => `<tr><td><div class="platform-name"><span class="platform-logo ${site.code === 'csg' ? 'green' : site.code === 'sgcc' ? 'orange' : ''}">${logo(site.code)}</span>${esc(site.name)}</div></td><td>公开公告（免登录）</td><td><span class="account-status">${esc(site.health_status === 'healthy' ? '正常' : site.health_status === 'unhealthy' ? '异常' : '未检查')}</span></td><td>${esc(beijingDateTime(site.last_checked_at))}</td><td><button class="batch-action" data-health-site="${site.id}">健康检查</button></td></tr>`).join('')}</tbody></table></div></section>`;
}
...
function renderOverviewLive() {
  ...
  const platformRows = platforms.map(site => `<div class="health-row"><span class="platform-logo ${site.code === 'csg' ? 'green' : site.code === 'sgcc' ? 'orange' : ''}">${site.code === 'csg' ? '南' : site.code === 'ecp' ? 'E' : '国'}</span><span class="health-name">${esc(site.name)}</span><span class="health-bar"><span style="width:${site.health_status === 'healthy' ? 100 : site.health_status === 'unhealthy' ? 20 : 0}%"></span></span><strong>${site.health_status === 'healthy' ? '正常' : site.health_status === 'unhealthy' ? '异常' : '未检查'}</strong></div>`).join('');
